...
1. Find the key `Enter_the_Application_Id_Here` and replace the existing value with the application ID (clientId) of `ms-identity-javascript-c2s1` app copied from the Microsoft admin center.
...
Make sure that your questions or comments are tagged with [`microsoft-entra-id` `msal-js` `ms-identity` `adal` `msal`].
